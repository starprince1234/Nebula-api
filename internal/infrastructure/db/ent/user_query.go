...
type UserQuery struct {
	...
	withMentorProjectApplications   *MentorProjectApplicationQuery
	withReviewedProjectApplications *MentorProjectApplicationQuery
	withAPIKeys                     *APIKeyQuery
	withAPIKeyAudits                *APIKeyAuditQuery
...
// QueryMentorProjectApplications chains the current query on the "mentor_project_applications" edge.
func (_q *UserQuery) QueryMentorProjectApplications() *MentorProjectApplicationQuery {
	query := (&MentorProjectApplicationClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QueryReviewedProjectApplications chains the current query on the "reviewed_project_applications" edge.
func (_q *UserQuery) QueryReviewedProjectApplications() *MentorProjectApplicationQuery {
	query := (&MentorProjectApplicationClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
...
// QueryAPIKeyAudits chains the current query on the "api_key_audits" edge.
func (_q *UserQuery) QueryAPIKeyAudits() *APIKeyAuditQuery {
	query := (&APIKeyAuditClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withMentorProjectApplications:   _q.withMentorProjectApplications.Clone(),
		withReviewedProjectApplications: _q.withReviewedProjectApplications.Clone(),
		withAPIKeys:                     _q.withAPIKeys.Clone(),
		withAPIKeyAudits:                _q.withAPIKeyAudits.Clone(),
...
// WithMentorProjectApplications tells the query-builder to eager-load the nodes that are connected to
// the "mentor_project_applications" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithMentorProjectApplications(opts ...func(*MentorProjectApplicationQuery)) *UserQuery {
	query := (&MentorProjectApplicationClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withMentorProjectApplications = query
	return _q
}

// WithReviewedProjectApplications tells the query-builder to eager-load the nodes that are connected to
// the "reviewed_project_applications" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithReviewedProjectApplications(opts ...func(*MentorProjectApplicationQuery)) *UserQuery {
	query := (&MentorProjectApplicationClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withReviewedProjectApplications = query
	return _q
}
...
// WithAPIKeyAudits tells the query-builder to eager-load the nodes that are connected to
// the "api_key_audits" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithAPIKeyAudits(opts ...func(*APIKeyAuditQuery)) *UserQuery {
	query := (&APIKeyAuditClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withAPIKeyAudits = query
	return _q
}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	var (
		...
		loadedTypes = [6]bool{
			...
			_q.withMentorProjectApplications != nil,
			_q.withReviewedProjectApplications != nil,
			_q.withAPIKeys != nil,
			_q.withAPIKeyAudits != nil,
		}
	)
	...
	if query := _q.withMentorProjectApplications; query != nil {
		if err := _q.loadMentorProjectApplications(ctx, query, nodes,
			func(n *User) { n.Edges.MentorProjectApplications = []*MentorProjectApplication{} },
			func(n *User, e *MentorProjectApplication) {
				n.Edges.MentorProjectApplications = append(n.Edges.MentorProjectApplications, e)
			}); err != nil {
			return nil, err
		}
	}
	if query := _q.withReviewedProjectApplications; query != nil {
		if err := _q.loadReviewedProjectApplications(ctx, query, nodes,
			func(n *User) { n.Edges.ReviewedProjectApplications = []*MentorProjectApplication{} },
			func(n *User, e *MentorProjectApplication) {
				n.Edges.ReviewedProjectApplications = append(n.Edges.ReviewedProjectApplications, e)
			}); err != nil {
			return nil, err
		}
	}
	...
	if query := _q.withAPIKeyAudits; query != nil {
		if err := _q.loadAPIKeyAudits(ctx, query, nodes,
			func(n *User) { n.Edges.APIKeyAudits = []*APIKeyAudit{} },
			func(n *User, e *APIKeyAudit) { n.Edges.APIKeyAudits = append(n.Edges.APIKeyAudits, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *UserQuery) loadMentorProjectApplications(ctx context.Context, query *MentorProjectApplicationQuery, nodes []*User, init func(*User), assign func(*User, *MentorProjectApplication)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(mentorprojectapplication.FieldMentorID)
	}
	query.Where(predicate.MentorProjectApplication(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.MentorProjectApplicationsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *UserQuery) loadReviewedProjectApplications(ctx context.Context, query *MentorProjectApplicationQuery, nodes []*User, init func(*User), assign func(*User, *MentorProjectApplication)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(mentorprojectapplication.FieldReviewedBy)
	}
	query.Where(predicate.MentorProjectApplication(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.ReviewedProjectApplicationsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *UserQuery) loadAPIKeys(ctx context.Context, query *APIKeyQuery, nodes []*User, init func(*User), assign func(*User, *APIKey)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(apikey.FieldOwnerUserID)
	}
...
func (_q *UserQuery) loadAPIKeyAudits(ctx context.Context, query *APIKeyAuditQuery, nodes []*User, init func(*User), assign func(*User, *APIKeyAudit)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(apikeyaudit.FieldActorUserID)
	}
	query.Where(predicate.APIKeyAudit(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.APIKeyAuditsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
