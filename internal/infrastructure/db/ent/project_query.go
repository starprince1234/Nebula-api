...
func (_q *ProjectQuery) QueryMemberships() *ProjectMemberQuery {
...
// QueryMentorApplications chains the current query on the "mentor_applications" edge.
func (_q *ProjectQuery) QueryMentorApplications() *MentorProjectApplicationQuery {
...
func (_q *ProjectQuery) WithMemberships(opts ...func(*ProjectMemberQuery)) *ProjectQuery {
	query := (&ProjectMemberClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// WithMentorApplications tells the query-builder to eager-load the nodes that are connected to
// the "mentor_applications" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *ProjectQuery) WithMentorApplications(opts ...func(*MentorProjectApplicationQuery)) *ProjectQuery {
	query := (&MentorProjectApplicationClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withMentorApplications = query
	return _q
}
...
func (_q *ProjectQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Project, error) {
	...
	if query := _q.withMemberships; query != nil {
		if err := _q.loadMemberships(ctx, query, nodes,
			func(n *Project) { n.Edges.Memberships = []*ProjectMember{} },
			func(n *Project, e *ProjectMember) { n.Edges.Memberships = append(n.Edges.Memberships, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withMentorApplications; query != nil {
		if err := _q.loadMentorApplications(ctx, query, nodes,
			func(n *Project) { n.Edges.MentorApplications = []*MentorProjectApplication{} },
			func(n *Project, e *MentorProjectApplication) {
				n.Edges.MentorApplications = append(n.Edges.MentorApplications, e)
			}); err != nil {
			return nil, err
		}
	}
...
func (_q *ProjectQuery) loadMemberships(ctx context.Context, query *ProjectMemberQuery, nodes []*Project, init func(*Project), assign func(*Project, *ProjectMember)) error {
...
func (_q *ProjectQuery) loadMentorApplications(ctx context.Context, query *MentorProjectApplicationQuery, nodes []*Project, init func(*Project), assign func(*Project, *MentorProjectApplication)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Project)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(mentorprojectapplication.FieldProjectID)
	}
	query.Where(predicate.MentorProjectApplication(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(project.MentorApplicationsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
