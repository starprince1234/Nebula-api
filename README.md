...
Nebula 是使用 Go 重构的现代 AI API 中转站。首期围绕学生、导师、老师三类用户，提供组织与项目管理、API Key 双级审批、模型与供应商配置、一次性 Key 领取、状态事件以及 OpenAI、Anthropic、Cohere Rerank 与 Google Gemini 原生协议代理。学生申请 Key 时只需提交模型 ID 和可选名称；老师通过顶部四节点向导补全模型信息、Binding 与启用状态，节点是唯一的步骤导航。进入基础配置时已有完整默认值，并可用居中的一键配置弹窗从供应商 `/v1/models` 和所选真实端点自动覆盖简介、类别、模态、能力与容量；Binding 新增和编辑也使用居中弹窗。
