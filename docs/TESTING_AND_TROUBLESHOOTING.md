...
| DataGrip 无法连接生产 PostgreSQL | PostgreSQL 只监听服务器本机，或 SSH tunnel 使用了变化的容器 IP | 使用服务器本机 `127.0.0.1:15432` 映射并在 DataGrip 启用 SSH tunnel；不要连接公网 5432 | 服务器执行 `ss -lntp '( sport = :15432 )'`，应看到 `127.0.0.1:15432`；DataGrip General 使用 `127.0.0.1:15432`，SSH 使用服务器 22 端口 |
