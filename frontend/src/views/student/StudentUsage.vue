...
function segments(key:StudentUsage['keys'][number]){return[...key.models.map(model=>({name:model.name,value:Number(model.credits)})),{name:'剩余额度',value:Number(remaining(key.quota,key.used,key.pending)),color:'#dfe3ec'}]}
</script>
  ...
  <section class="page">
    ...
    <div v-if="usage?.keys.length" class="grid two">
      <article v-for="key in usage.keys" :key="key.id" class="panel">
        ...
        <div class="usage-chart-card"><CreditDonut :segments="segments(key)" :total="Number(key.quota)" :label="`${key.name} 按模型拆分的本月 credit 用量`"/></div>
