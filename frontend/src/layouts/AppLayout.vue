...
onMounted(()=>{void refreshCount();window.addEventListener('focus',focused);window.addEventListener('nebula:refresh',refreshCount);eventController.value=new AbortController();openEvents((event,data)=>{if(event==='api_key.status_changed'||event==='models.common_changed')window.dispatchEvent(new CustomEvent('nebula:refresh',{detail:{event,data}}))},(state,count)=>{connection.value=state;failures.value=count;if(state==='connected')window.dispatchEvent(new CustomEvent('nebula:refresh',{detail:'sse-restored'}))},eventController.value.signal)});onBeforeUnmount(()=>{eventController.value?.abort();window.removeEventListener('focus',focused);window.removeEventListener('nebula:refresh',refreshCount)})
